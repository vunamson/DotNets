...
async def crawl_page(sem, session, object_id, max_retries=1):
    url = f"https://serenashirt.com/product/you-dont-need-to{object_id}-be-a-democrat-to-want-decency-back-shirt/"
...
async def main():
    while True : 
        start, end = 1600000,1700000
        object_ids = list(range(start, end + 1))

        sem = asyncio.Semaphore(5000)  # Giới hạn 30 request đồng thời
